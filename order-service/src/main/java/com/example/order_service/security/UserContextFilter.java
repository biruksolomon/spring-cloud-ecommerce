 ...
 * carries a valid X-Internal-Service-Token (the same token order-service
 * itself sends to product-service via ProductClientConfig). Only
 * api-gateway is configured with this token for inbound traffic, so a
 * request that reaches order-service directly with a forged X-User-Id/
 * X-User-Role pair is rejected outright instead of silently trusted.
 */
@Slf4j
@Component
